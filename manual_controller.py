...
if not os.path.exists(csv_file):
    with open(csv_file, 'w', newline='') as file:
        ...
        writer.writerow(["path_img_right", "path_img_left", "path_img_front", "angle", "steering_angle"])
...
def save_images_and_log(rgb_images, angle, steering_angle, img_num):
    ...
    with open(csv_file, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([right_img_path, left_img_path, front_img_path, angle, steering_angle])
...
# Función principal
def main():
    global img_num, recording, steering_angle
    robot = Car()
    driver = Driver()
    ...
    while robot.step() != -1:
        ...
        if (recording): 
            save_images_and_log([rgb_right, rgb_left, rgb_front], angle, steering_angle,  img_num)
            img_num += 1
